...
foreach ($byId as $id => &$node) {
  ...
  if ($pid === null) {
    ...
    if (isset($byId[(int)$pid])) $byId[(int)$pid]['children'][] = &$node;
    else $rootComments[] = &$node;
  }
}
unset($node);

function render_comment($c, $depth, $picture_id, $uploads, $isAdmin)
{
  ...
  <div class="comment" id="c-<?= (int)$c['comment_id'] ?>" data-depth="<?= $d ?>">
    ...
    <?php if (!empty($c['children'])): ?>
      <div class="c-children">
        <?php foreach ($c['children'] as $child) render_comment($child, $depth + 1, $picture_id, $uploads, $isAdmin); ?>
      </div>
    <?php endif; ?>
  </div>
<?php
        ...
        <div class="card">
          <div class="pad">
            ...
            <div id="comments">
              ...
              <?php else: ?>
                <?php foreach ($rootComments as $root) render_comment($root, 0, $picture_id, $paths->uploads, $isAdmin); ?>
