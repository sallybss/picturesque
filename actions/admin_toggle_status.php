...
// 1) Safety rails: do not allow blocking yourself, or blocking admins
...
if (!$user) {
  set_flash('err','User not found.'); header('Location: ../admin.php'); exit;
}
if ($targetId === $me) {
  set_flash('err','You cannot change your own status.'); header('Location: ../admin.php'); exit;
}
if ($user['role'] === 'admin') {
  set_flash('err','You cannot change another admin\'s status.'); header('Location: ../admin.php'); exit;
}

// 2) Toggle status: active <-> blocked (banned users cannot be changed here)
