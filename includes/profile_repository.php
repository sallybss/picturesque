...
class ProfileRepository extends BaseRepository
{
    ...
    {
        $sql = '
            SELECT display_name, avatar_photo, role
            FROM profiles
            WHERE profile_id = ?
        ';
    ...
    {
        $stmt = $this->db->prepare('SELECT role FROM profiles WHERE profile_id = ?');
    ...
    {
        $stmt = $this->db->prepare('SELECT login_email, password_hash FROM profiles WHERE profile_id = ? LIMIT 1');
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc() ?: null;
        $stmt->close();
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM profiles WHERE profile_id = ?');
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        $stmt->close();
    }
    ...
    {
        $stmt = $this->db->prepare('SELECT 1 FROM profiles WHERE login_email = ? LIMIT 1');
        $stmt->bind_param('s', $email);
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare('SELECT 1 FROM profiles WHERE login_email = ? AND profile_id <> ? LIMIT 1');
    ...
    {
        $stmt = $this->db->prepare('UPDATE profiles SET login_email = ? WHERE profile_id = ?');
    ...
    {
        $stmt = $this->db->prepare('UPDATE profiles SET login_email = ?, password_hash = ? WHERE profile_id = ?');
    ...
    {
        $stmt = $this->db->prepare('SELECT 1 FROM profiles WHERE email = ? AND profile_id <> ?');
        $stmt->bind_param('si', $email, $excludeId);
        $stmt->execute();
        $exists = (bool)$stmt->get_result()->fetch_row();
        $stmt->close();
    ...
    public function updateProfileWithEmailAndAvatar(
        int $id,
        string $name,
        ?string $email,
        ?string $avatar
        ...
        if ($avatar !== null && $email !== null) {
            $stmt = $this->db->prepare('
                UPDATE profiles
                SET display_name = ?, email = ?, avatar_photo = ?
                WHERE profile_id = ?
            ');
            ...
            $stmt = $this->db->prepare('
                UPDATE profiles
                SET display_name = ?, email = NULL, avatar_photo = ?
                WHERE profile_id = ?
            ');
    ...
    {
        if ($filename === null) {
            $stmt = $this->db->prepare('UPDATE profiles SET cover_photo = NULL WHERE profile_id = ?');
            $stmt->bind_param('i', $profileId);
        } else {
            $stmt = $this->db->prepare('UPDATE profiles SET cover_photo = ? WHERE profile_id = ?');
    ...
    public function deleteUserCascade(int $profileId): void
{
    $db = $this->db;

    $db->begin_transaction();
    try {
        $stmt = $db->prepare("DELETE FROM comments WHERE profile_id = ?");
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        $stmt->close();

        $stmt = $db->prepare("DELETE FROM likes WHERE profile_id = ?");
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        $stmt->close();

        $stmt = $db->prepare("SELECT picture_url FROM pictures WHERE profile_id = ?");
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        ...
        foreach ($pictures as $p) {
            if (!empty($p['picture_url'])) {
                $path = $uploadDir . basename($p['picture_url']);
                if (is_file($path)) {
                    @unlink($path);
                }
            }
        ...
        $stmt = $db->prepare("DELETE FROM pictures WHERE profile_id = ?");
        $stmt->bind_param('i', $profileId);
        $stmt->execute();
        $stmt->close();
